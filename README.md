...
基于 ESP32 和 MAX30102 光学传感器的便携式心率（BPM）与血氧饱和度（SpO2）实时监测系统。信号处理算法（滤波、峰值检测、血氧计算）均为手写实现，不依赖现成的信号处理库。
...
仅需手机 + ESP32 + MAX30102，不依赖 PC 图形界面。
...
如果你想从零了解每一行代码的原理和实现过程，请阅读 [Tutorial.md](Tutorial.md)，包含：

- 第一阶段：硬件接线与 I2C 通信
- 第二阶段：信号处理算法原理与实现（滤波、峰值检测、Beer-Lambert 定律）
- 第三阶段：Python 串口通信与实时可视化
